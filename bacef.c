...
static void update_watch_paths_prefix(watch_info_t *watches, int watch_count,
  ...
  for (int i = 0; i < watch_count; i++) {
    if (strncmp(watches[i].path, old_prefix, old_len) == 0 &&
        (watches[i].path[old_len] == '\0' || watches[i].path[old_len] == '/')) {
      char suffix[MAX_PATH_LEN];
      snprintf(suffix, sizeof(suffix), "%s", watches[i].path + old_len);
      snprintf(watches[i].path, MAX_PATH_LEN, "%s%s", new_prefix, suffix);
    }
  }
}
...
int copy_file(const char *src, const char *dest, const char *source_base,
  ...
  if (S_ISLNK(src_stat.st_mode)) {
    ...
    if (ensure_parent_dirs(dest) < 0) {
      perror("mkdir parents");
      return -1;
    }
...
int copy_directory_recursive(const char *src, const char *dest,
  ...
  while (!should_exit && (entry = readdir(dir)) != NULL) {
          ...
          if (event->mask & IN_CREATE) {
            ...
            if (event->cookie != 0) {
              ...
              if (slot >= 0) {
                ...
                snprintf(moves[slot].src_path, MAX_PATH_LEN, "%s", src_path);

                snprintf(moves[slot].dest_path, MAX_PATH_LEN, "%s", dest_path);
              } else {
...
int add_recursive_watches(int inotify_fd, const char *dir_path,
  ...
  if (wd >= 0) {
    if (*watch_count < MAX_WATCHES) {
      ...
      snprintf(watches[*watch_count].path, MAX_PATH_LEN, "%s", dir_path);
      (*watch_count)++;
    } else {
